...
class GoogleActionResponseV2 {
    ...
    /**
     * Implementation of generic withTable
     * Show a simple card to the response object
     * @param {string} title
     ...
     * @return {GoogleActionResponse} this
     */
    addTable(title, subtitle, columnHeaders, rowsText) {
        this.responseObj.payload.google.richResponse.items.push(
            {
                tableCard: new Table()
                    .setTitle(title)
    ...
    /**
     * Add ask for update permission to response
     * @param {string} intentName
     * @param {string} speech
     * @return {GoogleActionResponse}
     */
    addAskForNotificationPermission(intentName, speech) {
        this.tell('<speak>' + speech + '</speak>');
        this.responseObj.fulfillmentText = speech;
        _.set(this.responseObj, 'payload.google.expectUserResponse', true);
        let systemIntent = {
    ...
    /**
     * Add register update intent (a.k.a. Daily Updates)
     * @param {string} intent
     * @param {Array<any>?} parameters
     * @param {string?} frequency
     * @return {GoogleActionResponse} this
     */
    addAskForRegisterUpdate(intent, parameters, frequency) {
        if (_.isEmpty(intent)) {
        ...
        _.set(this.responseObj, 'payload.google.systemIntent', systemItemt);
        ...
        _.set(this.responseObj, 'payload.google.inputPrompt', inputPrompt);
...
class Table {
     ...
     * @return {Table}
     */
    addRow(cellsText, dividerAfter) {
        if (!this.rows) {
            this.rows = [];
        }
        if (!cellsText) {
            throw new Error('cellsText cannot be empty');
        }
        const cells = [];
        cellsText.map((text) => {
    ...
    addRows(rowsText) {
        if (!rowsText) {
            throw new Error('rowsText cannot be empty');
        }
        rowsText.map((cellText) => {
     ...
     * @return {Table}
     */
    addColumn(header, horizontalAlignment) {
