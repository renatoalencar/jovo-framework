...
class GoogleActionDialogFlowV2Request extends DialogFlowV2Request {
    ...
    getGoogleActionRequest() {
        ...
        return this.originalDetectIntentRequest.payload;
    }
    ...
    getRequestType() {
        ...
        if (this.getGoogleActionRequest()
            .isRegisterUpdateIntent()) {
            return BaseApp.REQUEST_TYPE_ENUM.ON_REGISTER_UPDATE;
        }
    ...
    setLocale(locale) {
        ...
        this.setLanguageCode(locale.toLowerCase());
        return this;
    }
    ...
    setAudioInterface() {
        ...
        for (let i = 0; i < this.getContexts().length; i++) {
            ...
            if (_.endsWith(c.name, 'actions_capability_screen_output') ||
                _.endsWith(c.name, 'google_assistant_input_type_keyboard') ||
                _.endsWith(c.name, 'actions_capability_web_browser')) {
